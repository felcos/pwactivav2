...
'next
	...
	case "setcookie"	
		...
		movil = trim(request.form("movil2"))
		cargo = ""
	  ...
	  END IF
		...
		if resp=0 then
			...
			response.Write("Error (setcookie) al crear la licencia...")
		end if
	...
	case "setcookie_movil"	
		...
		if resp=0 then
			...
			response.Write("Error (01m setcookie_movil) al crear la licencia...")
		end if
    ...
    <input type="hidden" name="frmAdminLogin" value="registro">
        <table border="0" cellpadding="4" align="center">
		  ...
		  <tr>
            ...
            <td colspan="4"><input type="tlf" name="movil2" maxlength="20" size="52" value="<%= request.Cookies("licencia")("movil") %>" class="form-control" required="required"></td>
          </tr>
          <tr>
...
$(document).ready(function() { 
	...
	function testForm(){
		...
		if (document.frm_registro.movil2.value=="") {ErrSubmit="<p class='destaca'>Tienes que introducir un número de teléfono movil.</p>"};
...
<% sub ComprobarLicencia() 
	...
	'nunca se deberia dar solo, si tiene bloqueadas las cookies
	...
	<% else %>
        ...
        <p>Error (cookies bloqueadas en su navegador) al guardar la cookie.</p>
        <p>&nbsp;</p>
...
<% sub EnviarLicencia %>
    ...
    <p>&iquest;Quieres dar de alta la licencia n&ordm; <%= intEntregadas + 1 %> para tu equipo?...<?= usuario %>---<?= movil %></p>
    <p>&nbsp;</p>
    <form action="/acceso/password.asp" method="post" target="_self" name="frm_confirm" id="frm_confirm">
		...
		<input type="hidden" name="movil2" value="<%= movil %>">
        <div class="botones">
