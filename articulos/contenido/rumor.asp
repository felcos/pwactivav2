        ...
        <tr>
            ...
            <td style="width: 12%; border: #F47C04; border-left-style: dotted; margin-left: 5px;" > 
                <div class="cuerpo" style=" margin-left: 5px;">
                    ...
                    <iframe name="I1" width="233" height="221" src="/dialogo/default.asp?id=<%= id_edificio %>&txt=<% Call NombreEdif(id_edificio) %>" border="0" frameborder="0" scrolling="no" allowtransparency="allowtransparency"></iframe>
...
<%
Set rsEdificios = Server.CreateObject("ADODB.Recordset")
sub NombreEdif(id_edificio)	
		sql= "SELECT ID, NOMBRE FROM inmuebles WHERE ID = " & id_edificio
		rsEdificios.Open sql, session("connPW")
		if rsEdificios.eof and rsEdificios.bof then
			%> el inmueble<%
		else
            Response.Write(rsEdificios("NOMBRE"))
		end if
		rsEdificios.close
end sub 
