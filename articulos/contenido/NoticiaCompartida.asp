<%
dim art_id
art_id=Request.QueryString("not") 
...
<meta property='og:image' content='https://www.propertyweb.eu/logoINPW.png'/>
<meta property='og:description' content='<%= resultado("TEXTO_NOTICIA") %>'/>
<meta property='og:url' content='<%= url_web2 %>' />
...
<!--#include virtual="/inc/body-header-2.asp" -->



<div id="contenedor_articulos" class="container">

	<h3 class="encabezado_webaoido">Articulo Compartido por un Suscriptor de PropertyWeb.eu</h3>
	<h1 class="titulo_noticia"><%= resultado("TITULO") %></h1>
<%
...
<div><script src="//platform.linkedin.com/in.js" type="text/javascript"> lang: es_ES</script><script type="IN/Share" data-href="<%=url_web %>" data-url="<%=url_web %>"></script></div>
<div class="fb-share-button" data-href="<%=url_web %>" data-layout="button_count" data-size="small" data-mobile-iframe="true"><a target="_blank" href="<%=url_web %>" class="fb-xfbml-parse-ignore">Compartir</a></div>
		...
		<span class="txt_gris_claro" style="font-weight:bold;font-size:12px;"><% if resultado("nacional")=1 then %>nacional<% else %>internacional<% end if %></span>&nbsp;&nbsp;<% if resultado("nacional")=1 then %><img src="/img/artic_nacional.png"><% else %><img src="/img/artic_internacional.png"><% end if %>
	</div>
    
    <p class="txt_gris_claro">Fecha Noticia: <%= resultado("Fecha_noticia") %></p>
...
<% if swMostrarDetalles then
	...
	strTextoHTML = resultado("TEXTO_NOTICIA")& chr(13)
  ...
  <% if resultado("ID_FUENTE")>0 then %>
    	<div id="separator_line" style="clear:both;"></div>
        <div class="fuente">
            Fuente: 
            <% if resultado("NUMERO_PAGINA")<>"" and resultado("NUMERO_PAGINA")<>"0" then %><br>&nbsp;P&aacute;g: <%= resultado("NUMERO_PAGINA") %></span><% end if %>
        </div>
...
<p style="font-size:smaller;"> El equipo editorial de PW no se hace responsable de las opiniones expresadas por sus lectores. </p>
	...
	<% if request.Cookies("dev")<>"" and not(modo_report) then %>
    	<div class="caja dev" style="margin-bottom:10px;">
            <li>Fecha Publicaci&oacute;n: <strong><%= resultado("FECHA_NOTICIA") %></strong></li>
            <li>Fecha Actualizaci&oacute;n: <strong><%= resultado("FECHA_ACTUALIZACION") %></strong></li>
            <div id="separator_line" style="clear:both;"></div>
...
<% else	'swMostrarDetalles
	...
	if request.Cookies("licencia")="" then
		...
		if ini_cliente=0 then	'es cliente activo
			...
			elseif abs(datediff("d", resultado("FECHA_ACTUALIZACION"), date))>7 and not(session("pw_ws").accesoRumores) then
				call AccesoSoloHoy("Web ha o&iacute;do...")
			
			elseif resultado("nacional")<>1 and not(session("pw_ws").accesoInternacional) then
