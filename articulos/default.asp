...
<div id="" class="container">
    ...
    <nav class="barraNav">
        <% if swVolver then %>
            <% if volver_simple then %>
                <a href="<%= url %>" class="btn blanco" <% if request.Cookies("dev")<>"" then %>data-toggle="popover" data-html="true" data-trigger="hover" data-placement="right" data-original-title="<%= url %>"<% end if %> ><span class="icon icon-arrow-left2"></span> <span class="lineLeft">Volver</span></a>
            <% else %>
                <form action="<%= url %>" method="post" id="frm_volver" style="display:inline-block;">
                    <% for each elto in request.Form
                        select case elto
                        case "origen"
                        case "not", "rum", "est", "ven", "sub", "dem", "t4a"	'"ope"
						case "ope"
							if request.Form("submit")<>"" then
								%><input type="hidden" name="<%= elto %>" value="<%= request.Form(elto) %>" /><%
								informa = informa & elto & ": " & request.Form(elto) & "<br>"
							end if
						case "dis"
							'if request.Form("submit")<>"" then
                            %><input type="hidden" name="<%= elto %>" value="<%= request.Form(elto) %>" /><%
                    ...
                    informa = "<ul>" & informa & "</ul>"
                    %>
                    <button type="submit" class="btn blanco" <% if request.Cookies("dev")<>"" then %>data-toggle="popover" data-html="true" data-trigger="hover" data-content="<%= informa %>" data-placement="right" data-original-title="<%= url %>"<% end if %> /><span class="icon icon-arrow-left2"></span> <span class="lineLeft">Volver</span></button>
                </form>
            <% end if %>
        <% else %>
            <button type="submit" class="btn blanco" disabled="disabled"/><span class="icon icon-arrow-left2"></span> <span class="lineLeft">Volver</span></button>
        <% end if %>
        <% if request.Cookies("dev")<>"" then %>
