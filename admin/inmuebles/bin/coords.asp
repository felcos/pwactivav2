...
'lat = replace(request.QueryString("lat"), ".", ",")
'lng = replace(request.QueryString("lng"), ".", ",")
id = request.QueryString("id")
lat = request.QueryString("lat")
lng = request.QueryString("lng")
place_id = request.QueryString("placeid")


if id="" then response.End()

sql = ""
if cstr(lat & "" & lng)="" then 
	if request.Cookies("admin")("confirmar_coordenadas")<>"" then sql = sql & "tiene_coords=0, "
	sql = sql & "lat=NULL, "
	sql = sql & "lng=NULL"
else
	if request.Cookies("admin")("confirmar_coordenadas")<>"" then sql = sql & "tiene_coords=1, "
	sql = sql & "lat=" & lat & ", "
	sql = sql & "lng=" & lng
end if
...
if place_id<>"" then
	sql = sql & ", "
	sql = sql & "place_id='" & place_id & "'"
end if

sql = "UPDATE inmuebles SET " & sql & " WHERE id=" & id
