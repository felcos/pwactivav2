...
if (sw_edif and sw_cc and sw_hot) then
	...
	if sw_edif then
		if sqlt<>"" then sqlt = sqlt & " OR "
		sqlt = sqlt & "id_tipo_inmueble=0"
		
		'sqlt = sqlt & " AND id IN ("
		'sqlt = sqlt & "SELECT ID_EDIFICIO FROM C_OPERACIONES WHERE ("
		'sqlt = sqlt & "(ID_PROVINCIA = 3 OR ID_PROVINCIA = 2) AND (seccion LIKE '%oficina%') AND "
		'sqlt = sqlt & "(FECHA_OPERACION >= CONVERT(DATETIME, '01/01/2000', 103)) AND (web_es <> 0)"
		'sqlt = sqlt & "(web_es <> 0)"
		'sqlt = sqlt & ") )"
	end if
...
<td class="peq" align="center" style="width:30px;"><input type="checkbox" id="tiene_coords_<%= rsBusq("id") %>" name="tiene_coords" value="1" <% if rsBusq("tiene_coords") then %>checked<% end if %> disabled="disabled" class="confirmar_coordenadas"/></td><!--  onclick="cambia_tienecoords(< %= rsBusq("id") %>)"  -->
