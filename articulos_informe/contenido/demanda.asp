...
<div id="contenedor_articulos">
	...
	<h3 class="encabezado_demandas">Anuncios de Demandas</h3>
...
<% if swMostrarDetalles then 
	if not modo_report then
		secc = session("secc")
		if secc="" then secc = "dem"
		insert_reg_articulo secc, "dem", pRS("ID")
	end if 
    ...
    <div class="fuente">
        <p>Fuente: <!--#include virtual="/lib/fuentes.asp" --></p>
        <% if pRS("NUMERO_PAGINA")<>"" and pRS("NUMERO_PAGINA")<>"0" then %><span>&nbsp;Pag: <%= pRS("NUMERO_PAGINA") %></span><% end if %>
	</div>
...
<% else	'swMostrarDetalles
	...
	if session("pw_ws").ClienteId>0 then
		call SinAcceso("Anuncios de Demandas")
	else
		call NoCliente
	end if
