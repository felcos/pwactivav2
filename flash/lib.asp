	...
	<div class="bloque">
	    <% do while not resultado.EOF
			...
			if ulwtb<>Resultado("ID_NOTICIA") then
				...
				num_titulo=num_titulo+1
				if Resultado("TipoComentario")="N" or Resultado("TipoComentario")="R" or Resultado("TipoComentario")="E" or Resultado("TipoComentario")="T4AC" then 

					if tipowtb="t4a" then
						%>
						
						<div  style="border-top: 1px solid blue;border-color:#858585;color:#10384E;font-weight: bold;margin: 0px 0px 0px 0px;"><a style="color:#195677;font-weight: bold;margin: 0px 0px 0px 0px;" href="<%= enlace %>" class="simplemodal"><% TituloWTBt(resultado) %></a></div>
<div style="margin: 1px 0px 0px 0px;"><img src="https://www.propertyweb.eu/img/icon_comments_74.png" height="16" style="color:#10384E;font-weight: bold;" /><a style="color:#195677;font-weight: bold;margin: 0px 0px 0px 0px;" href="<%= enlace %>" class="simplemodal"> Comentarios ... Comparte lo que piensas...</a></div>
						<% if (Resultado("Comentario")<>"-Like-") then %>
						
						<p style="border-top: 1px solid blue;border-color:#858585;color:#10384E;font-weight: bold;margin: 0px 0px 0px 33px;"><a style="color:#2F85B1;margin: 0px 0px 0px 0px;" href="<%= enlace %>" class="simplemodal">
						<%=Resultado("Comentario") %></a>
						</p>
						<% end if  %>
						<% 
					else
						%>
						<div style="border-top: 1px solid blue;border-color:#858585;color:#10384E;font-weight: bold;margin: 0px 0px 0px 0px;"><a style="color:#195677;font-weight: bold;margin: 0px 0px 0px 0px;" href="<%= enlace %>" class="simplemodal"><% TituloWTBn(resultado) %></a></div>
<div style="margin: 1px 0px 0px 0px;"><img src="https://www.propertyweb.eu/img/icon_comments_74.png" height="16" style="color:#10384E;font-weight: bold;" /><a style="color:#195677;font-weight: bold;margin: 0px 0px 0px 0px;" href="<%= enlace %>" class="simplemodal"> Comentarios ... Comparte lo que piensas...</a></div>
						<% if (Resultado("Comentario")<>"-Like-") then %>
						<p style="border-top: 1px solid blue;border-color:#858585;color:#10384E;font-weight: bold;margin: 0px 0px 0px 33px;"><a style="color:#2F85B1;margin: 0px 0px 0px 0px;" href="<%= enlace %>" class="simplemodal">
						<%=Resultado("Comentario") %></a>
					...
					if Resultado("TipoComentario")="O" then
						...
						%>
						<div  style="border-top: 1px solid blue;border-color:#858585;color:#10384E;font-weight: bold;margin: 0px 0px 0px 0px;"><a style="color:#195677;font-weight: bold;margin: 0px 0px 0px 0px;" href="<%= enlace %>" class="simplemodal"><% TituloWTBo(resultado) %></a></div>
<div style="margin: 1px 0px 0px 0px;"><img src="https://www.propertyweb.eu/img/icon_comments_74.png" height="16" style="color:#10384E;font-weight: bold;" /><a style="color:#195677;font-weight: bold;margin: 0px 0px 0px 0px;" href="<%= enlace %>" class="simplemodal"> Comentarios ... Comparte lo que piensas...</a></div>						
						<% 
							if (Resultado("Comentario")<>"-Like-") then %>
								<p style="border-top: 1px solid blue;border-color:#858585;color:#10384E;font-weight: bold;margin: 0px 0px 0px 33px;"><a style="color:#2F85B1;margin: 0px 0px 0px 0px;" href="<%= enlace %>" class="simplemodal">
...
apart_primero = true
%>
<div class="av2-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 15px; margin-top: 15px;">
    <% do while not resultado.EOF
		...
		'for i = 0 to resultado.fields.Count - 1
   		''	response.write "<p align='center' bgcolor='#eeeeee'>"
		''	   response.write resultado.fields(i).name
		''	   response.write resultado.fields(i).value
		''	response.write (i & "</p>" )
		'next
		'response.write resultado.fields(0).name
        ...
        %>
<div class="av2-card" style="padding: 15px; display: flex; align-items: flex-start; gap: 10px; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='var(--shadow-md)';" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='var(--shadow-sm)';">
    <div style="padding-top: 2px;">
        <input type="checkbox" name="<%= strin %>" value="<%= resultado("id") %>" style="accent-color: var(--accent); transform: scale(1.2);">
    </div>
    <div style="flex-shrink: 0;">
        <img src="<%=sFotoN %>" width="32" height="32" style="border-radius: 6px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); object-fit: cover;">
    </div>
    <div style="flex-grow: 1;">
        <a href="<%= enlace %>" class="simplemodal" style="font-weight: 600; color: var(--text-primary); text-decoration: none; font-size: 0.95rem; line-height: 1.4; display: block; margin-bottom: 5px;">
            <% Titulo(resultado) %>
        </a>
    </div>
</div>
