...
<% sub BloqueTitulos()	
	...
	<div class="bloque">
	    <% do while not resultado.EOF
			...
			%>
			<div class="articulo" ><input type="checkbox"    name="<%= strin %>" value="<%= resultado("id") %>" ><a href="<%= enlace %>" class="simplemodal"><img src="<%=sFotoN %>" width="25px" style="vertical-align:top;border-radius: 3px 3px 3px 3px;box-shadow: 2px 2px 10px #666;"/>&nbsp;&nbsp;&nbsp;<% Titulo(resultado) %></a></div>
					<% resultado.movenext
		loop %>
	</div>
<% end sub %>
