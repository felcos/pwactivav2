...
'end if
pFecha1 = date()
pFecha2 = DateAdd("m",10,pFecha1)
pFecha3 = DateAdd("m",8,pFecha1)
pFecha4 = DateAdd("m",-1,pFecha1)
pFecha5 = DateAdd("m",2,pFecha1)
...
<div class="row" >
    ...
    <div class="col-md-6"  >
        ...
        <style>
            .dialogbox1{
                background: url(ventana1x.png);
                position: absolute; left: 321px; top: 1px; z-index: 2;
                width: 150px;height: 100px;
            }
            .dialogbox2{
                background: url(ventana2.png);
                align-self: center;
                width: 570px;height: 221px;
                vertical-align:top;
            ...
            .dialogbox3{
                background: url(ventana3.png);
                align-self: center;
                width: 570px;height: 177px;
                vertical-align:top;
        ...
        <div  class="dialogbox1"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;wanna tell us 
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; something 
            <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;914 295 143  </div>
    ...
    <div  class="dialogbox3"> 
        <br>
        <h2 style="color:#1C6EA4">&nbsp;&nbsp;&nbsp;Quieres buscar en nuestra base de datos?</h2>
        <input id="txtSearch_N01" style="margin: 21px; width: 400px;" name="searchTerm_N01" placeholder="Escriba palabras clave a buscar..." class="form-control "  />
              
    </div>
   
        <h2 class="tit_box"><span class="icon icon-arrow-down-right2"></span>Empresas ¿quizas? buscando oficinas</h2>
        <%' Vencimientos	
        bloque="ven"
...
''''PROCESO AUTOMATICO VENCIMIENTOS DE CONTRATO

'' ***********************************************PARTE 1*****************************
	set procesoven1 = Server.CreateObject("ADODB.Recordset")
	set procesoven2 = Server.CreateObject("ADODB.Recordset")
	set procesoven3 = Server.CreateObject("ADODB.Recordset")
	set procesoven4 = Server.CreateObject("ADODB.Recordset")

        sql_vencim = "UPDATE TOP (10) OPERACIONES SET FECHA_PUBLICACION_VENCIMIENTO=CONVERT(DATETIME, '" & pFecha1 & "', 103) "
        sql_vencim = sql_vencim & "WHERE (FECHA_PUBLICACION_VENCIMIENTO>=CONVERT(DATETIME, '" & pFecha1 & "', 103)) AND  "
	sql_vencim = sql_vencim & " (FECHA_FIN BETWEEN  CONVERT(DATETIME, '" & pFecha3 & "', 103) AND CONVERT(DATETIME, '" & pFecha2 & "', 103)) and FECHA_FIN is not null "	
	sql_vencim = sql_vencim & "AND web_es <> 0"
        procesoven1.Open sql_vencim, session("connPW")	
'response.write(sql_vencim)
'response.write("<br>")
        ...
        sql_vencim = sql_vencim & "(FECHA_PUBLICACION_VENCIMIENTO BETWEEN  CONVERT(DATETIME, '" & pFecha1 & "', 103) AND CONVERT(DATETIME, '" & pFecha1 & "', 103)) "	
	...
	procesoven2.Open sql_vencim, session("connPW")	
'response.write(sql_vencim)
'response.write("<br>")

	if procesoven2.eof then 
		sql_vencim = "update OPERACIONES set FECHA_PUBLICACION_VENCIMIENTO = '" & pFecha5 & "' "
		sql_vencim = sql_vencim & " WHERE (FECHA_FIN BETWEEN CONVERT(DATETIME, '" & pFecha3 & "', 103) "
		sql_vencim = sql_vencim & "AND CONVERT(DATETIME, '" & pFecha2 & "', 103)) "
		sql_vencim = sql_vencim & "and FECHA_FIN is not null AND web_es <> 0 "
		procesoven3.Open sql_vencim, session("connPW")
'response.write(sql_vencim)
'response.write("<br>")
			...
			sql_vencim = sql_vencim & " (FECHA_FIN BETWEEN  CONVERT(DATETIME, '" & pFecha3 & "', 103) AND CONVERT(DATETIME, '" & pFecha2 & "', 103)) and FECHA_FIN is not null "	
			sql_vencim = sql_vencim & "AND web_es <> 0"
        		procesoven4.Open sql_vencim, session("connPW")	
...
'response.write("<br>")
	...
	'procesoven3.close 
	'procesoven4.close 
'**************** fin proceso automatico
'' ***********************************************PARTE 2*****************************
'todo lo de arriba pero con pfecha en lugar de pfecha1
'**************** fin proceso automatico
	...
	sql_vencim = sql_vencim & "AND web_es <> 0 "
        ...
        <% if 1=1 then %>
        <h2 class="tit_box"><span class="icon icon-arrow-down-right2"></span>Ops Inversión publicadas en agosto.</h2>
        <div class="box_blanco">
            <div  style="align-content: center;"><a href="/dealanalysis/"><!--#include virtual="/flash2/USO.htm" --></a></div>
        </div>
        <h2 class="tit_box"><span class="icon icon-arrow-down-right2"></span>Ops Alquiler/Traspaso publicadas en agosto</h2>
        <div class="box_blanco">
            <div  style="align-content: center;"><a href="/dealanalysis/"><!--#include virtual="/flash2/alq.htm" --></a></div>
        </div>
        <h2 class="tit_box"><span class="icon icon-arrow-down-right2"></span>Ops Alquiler/Traspaso publicadas en agosto</h2>
        <div class="box_blanco">
            <div  style="align-content: center;"><a href="/dealanalysis/"><!--#include virtual="/dealanalysis2/resumen/tabla.asp" --></a></div>
        </div>
        <% end if
         %>
    ...
    <div class="col-md-6">
        ...
        SQL_SELECT = "SELECT isnull(FOTOS,'/fotos/icono.png') as FOTOS, TITULO, ID, FECHA_ACTUALIZACION, TIPOSECCION AS APARTADO, TITULO_ING AS TITULO_AUX,"
        SQL_SELECT = SQL_SELECT & "icono_seccion,  isnull(FOTOS,'/fotos/icono.png') as FOTOS  FROM C_NOTICIAS_INMOBILIARIAS "
        ...
        <h2 class="tit_box"><img src="icono-loc.png" width="56" style="width: 56;">&nbsp;Passeig de Gracia, 89</h2>
                ...
                <div class="video-js-box">
                    <video 
                        ...
                        poster="PV110_player/preview.png">
                            <source src="pg89.mp4" type='video/mp4; codecs="avc1.42E01E, mp4a.40.2"' />                  
                            <!-- Flash Fallback. Use any flash video player here. Make sure to keep the vjs-flash-fallback class. -->
                            <object id="flash_fallback_1" class="vjs-flash-fallback" width="500" height="250" type="application/x-shockwave-flash"
                            ...
                            <img src="PV110_player/preview.png" width="500" height="250" alt="Poster Image" title="No video playback capabilities." />
                            </object>
                    </video>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                        <a  href="/info/edificio/" onclick="$('#frm1_pg89').submit();return false;" >
                            <div class="inm_nombre"><span style="color: #ff6701;" class="icon-office"></span>&nbsp;¿Quieres saberlo todo?</div>
                        </a>
                        <a href="https://www.propertyweb.eu/articulos/?ope=34931&seltipo=edif&frmInfo_tipo=edif&frmInfo_busq=PASEO%20GRACIA%2089&historico_alquiler=on&historico_inversion=on&historico_noticias=on&historico_rumores=on&historico_estudios=on&secc=edif&origen=edif:5921&id_edificio=5921&edificio=GRACIA%2089&calle=GRACIA&l=BARCELONA&numerocalle=89&d=PASEO%20GRACIA%2089"  >
                        ...
                        <a href="https://www.propertyweb.eu/dealanalysis/default.asp?xOrigen=1" ><div class="inm_nombre"><span style="color: #ff6701;" class="icon-equalizer"></span>&nbsp;¿Quieres ver operaciones?</div></a>
        ...
        SQL_SELECT = "SELECT isnull(FOTOS,'/fotos/icono.png') as FOTOS, ID, TITULO,TITULO_pt AS TITULO_AUX, FECHA_ACTUALIZACION, seccion AS APARTADO, icono_seccion "
...
sql = "SELECT top 20 * FROM View_Noticias_Comentrios where Id_Noticia!=55 Order by fecha desc , Comentario desc "
...
<% end if%>
        ...
        sql = "SELECT isnull(FOTOS,'/fotos/icono.png') as FOTOS, TITULO, Id_Concurso AS ID, FECHA_ACTUALIZACION, tipo_concurso AS APARTADO, icono_seccion "
...
<script type="text/javascript">
    ...
    function cambiopais() {
        ...
        var frm_destino = "/dealanalysis2/q/localidades_buscar_con_id.asp"
    ...
    $(document).ready(function(){
        ...
        $("#frm_deal").submit(function(){
            ...
            $.ajax({
                type: "POST",
                dataType: "html",
                url: "/dealanalysis2/resultados/resultados.asp",
    ...
    <!--#include virtual="/dealanalysis2/autocomplete.asp" -->
