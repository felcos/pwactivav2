...
'response.write(request.Form("sec") &" "& request.Form("operacion"))
		...
		$.each(agentes_todos, function(jj, agente) {
			if (agente.ID == operacion.ID) {
				...
				if (agente.logotipo === null) {
					...
					img =  new Image();
					img.src = "/_inc/javier/img/empresas_/" + agente.logotipo;
					images.push(img);
				}
			}
		});
	...
	function contenidoInfoBox( operacion ) {
		...
		for (var jj=0; jj<operacion.agentes.length; jj++) {
			if (operacion.agentes[jj].logotipo === null) {
				...
				res = res + '<img src="/_inc/javier/img/empresas_/' + operacion.agentes[jj].logotipo + '">';
