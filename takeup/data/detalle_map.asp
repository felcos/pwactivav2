...
<%
resp_reg = session("pw_ws").Reg(request.ServerVariables("PATH_INFO"), request.queryString, request.QueryString, request.ServerVariables("HTTP_REFERER"))

if not session("pw_ws").accesoTakeUp then %>
...
resp_regArticulo = session("pw_ws").RegArticulo("takeup", "dis", cdbl(rsInm("id")))
