...
if sel_year="" then sel_year = "2026"
				...
				if (inm) {
					...
					<% if sel_year="2026" then %>
						...
						if (agente.logotipo === null) {
							...
							res = '<img style="max-width:25px;width:100%" src="/_inc/javier/img/empresas/' + agente.logotipo + '">';
						}
					...
					<% else %>
						...
						if (agente.agencia_logotipo === null) {
							...
							res = '<img style="max-width:25px;width:100%" src="/_inc/javier/img/empresas/' + agente.agencia_logotipo + '">';
						}
						$("#inm_" + inm.id + "-intermediario").append(res);
		...
		if (inmueble.agentes) {
			for (var jj=0; jj<inmueble.agentes.length; jj++) {
				if (inmueble.agentes[jj].logotipo === null) {
					...
					res = res + '<img style="max-width:25px;width:100%" src="/_inc/javier/img/empresas/' + inmueble.agentes[jj].logotipo + '">';
				}
			};
		}
		res = res + '</td></tr>';
		...
		$("#sup-ocup").html("0");
			...
			success: function(recibe, txtStatus, jqSHR) {
				...
				if (data.disponible==null) {
					...
					$("#informa_resultados #informa-total-sup-busq").html(", con un total de " + formatear(data.disponible) + " m&sup2;");
				}
			}
		})
		$.ajax({
...
<% end if %>
	...
	<% if request.Form("year")="2026" then %>
    ...
    <div class="dispB">
        <table class="tbDispon">
            <tr>
                ...
                <% if sel_year="2026" then %><td class="tbDisp-Min"><%= c_disp_min %></td><% end if %>
