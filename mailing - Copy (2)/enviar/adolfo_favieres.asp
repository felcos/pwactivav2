...
Resultados de la b&uacute;squeda - 'ALCALA MAGNA'<br>
<%
modo_report = true
...
<div id="main_read">
<%
sqlW = "(PALABRAS_CLAVES LIKE '%alcala%' AND PALABRAS_CLAVES LIKE '%magna%' AND "
sqlW = sqlW & "PALABRAS_CLAVES LIKE '%CENTRO%' AND PALABRAS_CLAVES LIKE '%COMERCIAL%')"
...
'Operaciones	
	sqlW = "NOMBRE_ZONA LIKE '%alcala magna%' OR "
	sqlW = sqlW & "inmueble LIKE '%alcala magna%' "
	
	sql = "SELECT * FROM C_OPERACIONES WHERE (web_es=1 AND (FECHA_ACTUALIZACION BETWEEN " & desde & " AND " & hasta & ") AND (" & sqlW & ")) ORDER BY FECHA_ACTUALIZACION DESC"
