...
%>
<title>Property Web - Operaciones</title>
</head>
...
<div class="seccion_tit" style="margin-bottom:32px; border-bottom:1px solid #e1e1e1;">
	<p style="margin-top:4px; margin-bottom:4px;"><strong>Operaciones de Centros Comerciales</strong></p>
    <p style="margin-top:4px; margin-bottom:4px; font-size:80%;">desde febrero-2015</p>
</div>
<%' Operaciones	
bloque="ope"
strin="ope"	
...
SQL_WHERE = SQL_WHERE & "(seccion LIKE '%centros comerciales%') AND (ID_PAIS = 1) AND "

SQL_WHERE = SQL_WHERE & "(FECHA_PUBLICACION >= CONVERT(DATETIME, '01/02/2015', 103) AND "
SQL_WHERE = SQL_WHERE & "FECHA_ACTUALIZACION <= CONVERT(DATETIME, '10/09/2015', 103)) "	


SQL_ORDER = "ORDER BY FECHA_PUBLICACION "
%>
<% 'nacional			
SQL = SQL_SELECT & SQL_WHERE & "AND web_es <> 0 " & SQL_ORDER

test_inyeccion_sql sql
resultado.Open sql, session("connPW") %>
<div class="ubicacion">
	<div class="apartado"><img src="/img/flash/apunta.gif"><strong>&nbsp;Centros Comerciales</strong></div>
<% apart=""
