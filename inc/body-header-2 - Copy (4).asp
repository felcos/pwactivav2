								...
								<form action="<%= url %>" method="post" id="frmInfo">
									<ul>
										...
										<li>
											...
											<label for="frmInfo_edif" id="lblInfo_edif" class="lblInfo <% if frmInfo_tipo="edif" then %>activo<% end if %>"><span class="icon-office"></span> <% if ver_contadores then %>Edif. o Dir. <span data-toggle="contador_leidos" data-content="edif"><%= ubound(filter(ArticulosLeidos, "edif"))+1 %></span>+<span data-toggle="contador_leidos" data-content="dir"><%= ubound(filter(ArticulosLeidos, "dir"))+1 %></span>+<span data-toggle="contador_leidos" data-content="zona"><%= ubound(filter(ArticulosLeidos, "zona"))+1 %></span><% else %>Edificio/Nave Industrial o Direcci&oacute;n<% end if %></label>
											<% if not(session("pw_ws").accesoInfoEdificio) then %><span class="icon-lock"></span><% end if %>
										</li>
										<li class="indent">
