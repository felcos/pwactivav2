...
if frmInfo_tipo="" then
	...
	if instr(request.ServerVariables("URL"), "/ni/")>0 then frmInfo_tipo="ni"
...
<script type="text/javascript">
	$(document).ready(function() {    
		$('.button').on('click', function(){
			...
			$('#gifLoader').html('<div class="loading"><img src="/images/loader.gif" alt="loading" /><br/>Un momento, por favor...</div>');
			...
			<div class="col-sm-9 izquierda">
				<div class="row">
					...
					<div class=" col-sm-12 col-md-4 logoPW">
						<a href="/"><img src="/_inc/javier/img/logoPWnew3.png" class="logo"></a> 
							...
							<div class="col-sm-6 colInfo" id="head_div_info">
								...
								<form action="<%= url %>" method="post" id="frmInfo">
									<ul>
										...
										<li>
											<input name="frmInfo_tipo" type="radio" class="infoRadio frmInfo_tipo button" value="cc" id="frmInfo_cc" <% if frmInfo_tipo="cc" then %>checked<% end if %>/>
										...
										<li>
											<input name="frmInfo_tipo" type="radio" class="infoRadio frmInfo_tipo button" value="ni" id="frmInfo_ni" <% if frmInfo_tipo="ni" then %>checked<% end if %>/>
										...
										<li class="indent">
											<input name="frmInfo_tipo" type="radio" class="infoRadio frmInfo_tipo button" value="nidisp" id="frmInfo_nidisp" <% if frmInfo_tipo="nidisp" then %>checked<% end if %>/>
											<label for="frmInfo_nidisp" id="lblInfo_nidisp" class="lblInfo <% if frmInfo_tipo="nidisp" then %>activo<% end if %>"><span class="icon-checkmark"></span> Disponibilidad<% if ver_contadores then %> <span data-toggle="contador_leidos" data-content="dis"><%= ubound(filter(ArticulosLeidos, "dis"))+1 %></span><% end if %></label>
											<% if not(session("pw_ws").accesoDisponibilidad) then %><span class="icon-lock"></span><% end if %>
										</li>
										<% 'end if %>
										<li>
											<input name="frmInfo_tipo" type="radio" class="infoRadio frmInfo_tipo button" value="hot" id="frmInfo_hot" <% if frmInfo_tipo="hot" then %>checked<% end if %>/>
										...
										<li>
											<input name="frmInfo_tipo" type="radio" class="infoRadio frmInfo_tipo button" value="edif" id="frmInfo_edif" <% if frmInfo_tipo="edif" then %>checked<% end if %>/>
											<label for="frmInfo_edif" id="lblInfo_edif" class="lblInfo <% if frmInfo_tipo="edif" then %>activo<% end if %>"><span class="icon-office"></span> <% if ver_contadores then %>Edif. o Dir. <span data-toggle="contador_leidos" data-content="edif"><%= ubound(filter(ArticulosLeidos, "edif"))+1 %></span>+<span data-toggle="contador_leidos" data-content="dir"><%= ubound(filter(ArticulosLeidos, "dir"))+1 %></span>+<span data-toggle="contador_leidos" data-content="zona"><%= ubound(filter(ArticulosLeidos, "zona"))+1 %></span><% else %>Edificio o Direcci&oacute;n<% end if %></label>
											
											<% if not(session("pw_ws").accesoInfoEdificio) then %><span class="icon-lock"></span><% end if %>
										</li>
										<li class="indent">
											<input name="frmInfo_tipo" type="radio" class="infoRadio frmInfo_tipo button" value="disp" id="frmInfo_disp" <% if frmInfo_tipo="disp" then %>checked<% end if %>/>
											<label for="frmInfo_disp" id="lblInfo_disp" class="lblInfo <% if frmInfo_tipo="disp" then %>activo<% end if %>"><span class="icon-checkmark"></span> Disponibilidad<% if ver_contadores then %> <span data-toggle="contador_leidos" data-content="dis"><%= ubound(filter(ArticulosLeidos, "dis"))+1 %></span><% end if %></label>
											<% if not(session("pw_ws").accesoDisponibilidad) then %><span class="icon-lock"></span><% end if %>
										</li>
                                        					<li class="indent">
											<input name="frmInfo_tipo" type="radio" class="infoRadio frmInfo_tipo button" value="takeup" id="frmInfo_takeup" <% if frmInfo_tipo="takeup" then %>checked<% end if %>/>
										...
										<li>
											<div class="inputField">
												<div class="input-group info-buscar" id="info-buscar" style="display:none; background:#CCC;">
													<input type="text" class="form-control" value="<%= trim(frmInfo_busq) %>" placeholder="" id="frmInfo_busq" name="frmInfo_busq" disabled="disabled" readonly  autocomplete="off">
												...
												<div id="gifLoader" style="margin: 5px; color:orange; align-items: center;"  ></div> 
												...
												<div class="tip-caja fade right in " role="" id="frmInfo-tip">
													...
													<div class="tip-content" id="frmInfo-tip-content">Por favor, introduce el texto aqui...  </div>
												</div>
				...
				<div class="user hidden-xs">
					<ul>
						...
						<li><span class="icon-user-tie"></span> Tu gestor:
							<p>Andy G.<% if 1=2 then%> Salazar<% end if %></p>
						</li>
						<% else	'dev %>
						<li class="informa">
