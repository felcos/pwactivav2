...
<div id="<%= div_mapa %>" style="width:275px; height:225px;"></div>
...
<!-- detalles op. : INI -->
<p><b>Tipo Operaci&oacute;n: <%= lcase(pRS("TIPOOPERACION")) %></b></p>
<% if 1=2 then %>
...
do while not rsDetalles.eof %>
  ...
  <tr>
    ...
    <td></td>
    <td><%= formatNumber(importe,2) %> &nbsp; <%= lcase(pRS("tipoprecio")) %></td>
    <td></td>
    <% if pRS("ID_TIPO_OPERACION")=2 then %>
