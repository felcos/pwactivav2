...
sqlW = " (web_es<>0) AND id IN (SELECT * FROM _rafaelaviles)"
'sqlW = " (web_es<>0 AND id IN (SELECT * FROM _operaciones_drago))"

sql = "SELECT C_OPERACIONES.*, C_OPERACIONES.seccion AS APARTADO FROM C_OPERACIONES "
sql = sql & "WHERE (" & sqlW & ") "
...
if ErrMesage="" then
	...
	'if limite<resultado.recordcount then ErrMesage=Resultado.recordcount & " Art�culos Encontrados .L�mite = " & limite & "<br>Depure la B�squeda"
