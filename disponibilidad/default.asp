...
<div class="PwTabs">
	...
	<div class="tab-content">
		...
		<!--#include virtual="/disponibilidad/inc/filtros.asp" -->
...
//				}
				...
				//console.log("inm:"+ inmueble.id + " fecha:"+Date.parse(fecha_texto)+" hoy:"+Date.parse(fecha_texto2));
				var marker = new google.maps.Marker({
					id: inmueble.id,
					map: map, 
			...
			$.each(agentes_todos, function(jj, agente) {
				...
				if (inm) {
					...
					if (agente.logotipo === null) {
						...
						res = '<img style="max-width: 33px; max-height:25px;" src="/_inc/javier/img/empresas/' + agente.logotipo + '">';
					}
					$("#inm_" + inm.id + "-intermediario").append(res);
		...
		if (inmueble.agentes) {
			for (var jj=0; jj<inmueble.agentes.length; jj++) {
				if (inmueble.agentes[jj].logotipo === null) {
					...
					res = res + '<img style="max-width: 33px; max-height:25px;" src="/_inc/javier/img/empresas/' + inmueble.agentes[jj].logotipo + '" ';
				}
			};
		}
		res = res + '</td></tr>';
