...
public sub test_inyeccion_sql(rSql)	
	IF 1=2 THEN
	if not session("pw_ws").TestSQL(rSQL) then
		session("pw_ws").Alerta rSQL, request.Form, request.QueryString, request.ServerVariables("HTTP_REFERER")
		response.End()
	end if
	END IF
	...
	if not(cPasa) then
		...
		'if application("servidor_web") = "navia" then
			response.End()
		'end if
	end if
	'END IF	
