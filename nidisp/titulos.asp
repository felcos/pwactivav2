...
<input type="hidden" data-form="busq" name="frmInfo_busq" value="<%= frmInfo_busq %>">
...
'' Para traer una variable aqui se debe meter en nidisp/inc/filtros.asp
'' for each elto in request.Form 
''  response.write(elto )
''  response.write(busqueda_ver)
''  response.write(": "+request.Form(elto))
''  response.write("---")
'' next 
...
total_superficie = 0
total_superficie_p = 0
total_p = 0
total_superficie_pr = 0
total_pr = 0
ids = ""
...
sqlw = calcular_sqldni(true)
	
sql = "SELECT * FROM dirs_w_inmuebles WHERE (" & sqlw & ") ORDER BY "
'sql = "SELECT * FROM dirs_w_inmuebles WHERE (" & calcular_sqldni(true) & ") ORDER BY "
...
$(document).ready(function() {
  ...
  <% if request.Form("ordenando")="" then %>
	...
	<% if counter>0 then %>
		$("#informa_resultados .tit_metros").html(", <table><tr><td><%= FormatNumber(total_p, 0) %> inmuebles en Proyecto, con <%= FormatNumber(total_superficie_p, 0) %> m&sup2;</td></tr><tr><td><%= FormatNumber(total_pr, 0) %> inmuebles en Renovación, con <%= FormatNumber(total_superficie_pr, 0) %> m&sup2;</td></tr><tr><td><%= counter-total_p-total_pr %> inmuebles Disponibles, con <%= FormatNumber(total_superficie-total_superficie_p, 0) %> m&sup2; </td></tr></table>");
	<% else %>
	...
	<% case else %>
		$("#informa_resultados .tit_numero").html("Total " + <%= counter %> + " inmuebles, con <%= FormatNumber(total_superficie, 0) %> m&sup2;");
		//preguntaSiguiente(1);
		//$(".contTodo").css({"left": "-100%" });
	...
	$.ajax({
		type: "POST",
		url: "/nidisp/data/total_registrados.asp",
		...
		if (fila.find(".tb-despliega").hasClass("activo")) {
			...
			if ( $("#inm_" + id + "-detalles").html()=="" ) {
				$.ajax({
					type: "POST",
					//async: false,
					url: "/nidisp/data/detalle.asp",
					data: {'id':id, 'secc':'nidisp'},
...
function calcular_sqldni(swFiltros) 
	dim tmp_sql
	...
	sqlni = ""
	sqlni = sqlni & " and ( id_tipo_inmueble=0 )"
	sqlni = sqlni & " AND (id_tipo_edificio is not null "
	'sqlni = sqlni & " or id_tipo_edificio=2 "
	'sqlni = sqlni & " or id_tipo_edificio=3 "
	'sqlni = sqlni & " or id_tipo_edificio=4 "
	sqlni = sqlni & " or id_tipo_edificio=5 "
	sqlni = sqlni & " or id_tipo_edificio=6 "
	'sqlni = sqlni & " or id_tipo_edificio=7 "
	'sqlni = sqlni & " or id_tipo_edificio=10 "
	'sqlni = sqlni & " or id_tipo_edificio=11 "
	sqlni = sqlni & " or id_tipo_edificio=12 "
	sqlni = sqlni & " or id_tipo_edificio=14 "
	'sqlni = sqlni & " or id_tipo_edificio=15 "
	sqlni = sqlni & " or id_tipo_edificio=17 "
	'sqlni = sqlni & " or id_tipo_edificio=18 "
	'sqlni = sqlni & " or id_tipo_edificio=19 "
	sqlni = sqlni & "  ) "

	tmp_sql = tmp_sql & sqlni
...
'3	PARQUE COMERCIAL
'4	NAVE COMERCIAL
'5	POLIGONO INDUSTRIAL
'6	NAVE INDUSTRIAL
'7	CENTRO EMPRESARIAL
'8	CAMPUS EMPRESARIAL
'9	CENTRO DE NEGOCIOS
'10	PARQUE TECNOLOGICO
'11	CENTRO TECNOLOGICO
'12	CENTRO LOGISTICO
'14	CENTRO DE TRANSPORTES
'15	COMPLEJO DE OFICINAS
'17	PARQUE INDUSTRIAL
'18	COMPLEJO EMPRESARIAL
'19	CIUDAD EMPRESARIAL
'20	LOCAL COMERCIAL
'23	LOCAL
	...
	tmp_sql = tmp_sql & " AND (  " 
	tmp_sql = tmp_sql & " NOMBRE_CALLE LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR NOMBRE_ZONA LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR TIPO_ZONA LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR NOMBRE LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR LOCALIDAD LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR PROVINCIA LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR PAIS LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR DIR1 LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR DIR2 LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR DIR3 LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR DIR4 LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR DIR5 LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR CODIGO_POSTAL LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR TIPO_INMUEBLE LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR SECCION LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR SUBZONA LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR NOMBRE_ALT LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " OR TIPO_DIRECCION LIKE '%" & frmInfo_busq & "%' "
	tmp_sql = tmp_sql & " ) "
	end if 
	if swFiltros then
		if localidad<>"" then
...
function BusquedaVacia()	
	...
	rsBusq.open "SELECT COUNT(*) AS nn FROM dirs_w_inmuebles WHERE " & calcular_sqldni(false), session("connPW")
...
sub Linea()	
	...
	xEnConstruccion=rsBusq("EnConstruccion")
	xEnProy=rsBusq("EnProy")
	xEnRenov=rsBusq("EnRenov")

	if xEnProy then
		total_superficie_p = total_superficie_p + cdbl(rsBusq("disponible_max"))
		total_p=total_p+1
	end if 

	if xEnRenov then

		total_superficie_pr = total_superficie_pr + cdbl(rsBusq("disponible_max"))
		total_pr=total_pr+1
	end if
...
''		if c_fecha_edif>ActualYYYY2 then 
''			total_superficie_p = total_superficie_p + cdbl(rsBusq("disponible_max"))
''			total_p=total_p+1
''		end if 
''	else 
''		if c_fecha_edif>ActualYYYY  then 
''			total_superficie_p = total_superficie_p + cdbl(rsBusq("disponible_max"))
''			total_p=total_p+1
''		end if 
''	end if 
''	if  c_fecha_renov>10000 then 
''		if c_fecha_renov>ActualYYYY2 then 
''			total_superficie_pr = total_superficie_pr + cdbl(rsBusq("disponible_max"))
''			total_pr=total_pr+1
''		end if 
''	else 
''		if c_fecha_renov>ActualYYYY then 
''			total_superficie_pr = total_superficie_pr + cdbl(rsBusq("disponible_max"))
''			total_pr=total_pr+1
''		end if 
''	end if 
...
<div class="dispA" data-id="<%= rsBusq("id") %>" <%  
	if xEnConstruccion  then  %> style="background-color:#CCCCCC;" <% end if 
	 %>
	>
    ...
    <div class="dispB">
        <table class="tbDispon">
            <tr>
				<td class="tbDisp-Plta">
					<% if xEnConstruccion   then 
					 %> <img src="/img/underc.png" width="25"/> <% end if 
					 %>
				</th>
