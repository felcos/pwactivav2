...
<html lang="es">
<head>
	<title>PropertyWeb - IL Disponibilidad</title>
...
<a id="scrollmap"></a>
<div class="miga"><h2 class="tit_miga02">Industrial/Logistica Disponibilidad<% if request.Cookies("dev")<>"" then %> &nbsp; [<a href="/nidisp/">reset</a>]<% end if %></h2></div>

<div class="tit_resultados"><h3 class="tit_metros">Buscando:<%=frmInfo_busq %> </h3></div>
...
<!--include virtual="/nidisp/preguntas_jp.asp" -->
<div class="PwTabs">
	...
	<div class="tab-content">
		...
		<!--#include virtual="/nidisp/inc/filtros.asp" -->
        
		<div class="tab-pane <% if settab="map" then %>active<% end if %>" id="map">
			<!--#include virtual="/nidisp/inc/diapositivas.asp" -->
		...
		<div class="tab-pane <% if settab="list" then %>active<% end if %>" id="list">
			...
			<div class="caja-operaciones">
            	...
            	<div class="divDispon clearfix">
                    ...
                    <div id="div_titulos"><!--include virtual="/nidisp/titulos.asp" --></div>
                    </form>
            	</div>
            </div>
...
<script>
	...
	function CargaAgencias() {
		...
		$("#dropdown-agencias").load(
			"/nidisp/data/select_agencias.asp", 
			$("#frm_preguntas").serialize(), 
			function(recibe) {
	...
	function EdificiosSubzona() {
		...
		$.ajax({
			url: "/nidisp/data/subzona-edificios.asp",
			data: datos,
			type: "POST",
			success: function(recibe) {
				//console.log(recibe);
	...
	function CambiaSubzona(id) {
		...
		$.ajax({
			url: "/nidisp/data/subzonas-coordenadas.asp",
			data: "id=" + id,
			type: "POST",
			success: function(recibe) {
				//console.log(recibe);
...
//				}
			...
			$.each(agentes_todos, function(jj, agente) {
				...
				if (inm) {
					...
					if (agente.logotipo === null) {
						...
						res = '<img style="max-width: 33px;" src="/_inc/javier/img/empresas/' + agente.logotipo + '">';
					}
					$("#inm_" + inm.id + "-intermediario").append(res);
	...
	function mapalista(inm) {
		...
		*/
		$.ajax({
			type: "POST",
			url: "/nidisp/data/detalle_map.asp",
