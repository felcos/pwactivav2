...
case "otc"
	sql = sql & "(OPERACIONES.ID_PROVINCIA <> 2) and (OPERACIONES.ID_PROVINCIA <> 3)"	
end select
...
<% do until rs.EOF 
	...
	select case secc
	case "oficinas", "locales comerciales"
...
sub MadridBarcelona(rSec) 
	...
	sql = "SELECT CASE WHEN LOCALIDADES.id_provincia = 2 THEN 2 ELSE CASE WHEN LOCALIDADES.id_provincia = 3 THEN 3 ELSE 10000 END END as id_provincia,  "
	sql = sql & "COUNT(DISTINCT OPERACIONES_DETALLE.id_operacion) AS ops, SUM(OPERACIONES_DETALLE.Superficie) AS sup "
	sql = sql & "FROM OPERACIONES INNER JOIN "

	sql = sql & "OPERACIONES_DETALLE ON OPERACIONES.ID = OPERACIONES_DETALLE.id_operacion INNER JOIN "
	sql = sql & "secciones_operaciones ON OPERACIONES_DETALLE.id_seccion = secciones_operaciones.ID LEFT OUTER JOIN "
	sql = sql & "LOCALIDADES ON OPERACIONES.ID_LOCALIDAD = LOCALIDADES.id "
	...
	sql = sql & "GROUP BY CASE WHEN LOCALIDADES.id_provincia = 2 THEN 2 ELSE CASE WHEN LOCALIDADES.id_provincia = 3 THEN 3 ELSE 10000 END END "
	'FC
