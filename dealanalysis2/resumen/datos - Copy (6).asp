...
sql = sql & "WHERE "
sql = sql & "(web_es = 1) and (OPERACIONES.ID_PROVINCIA <> 0) and "
...
sql = sql & "(OPERACIONES.FECHA_OPERACION BETWEEN CONVERT(DATETIME, '" & r_year & "-01-01 00:00:00', 102) AND CONVERT(DATETIME, '" & r_year & "-12-31 00:00:00', 102)) AND "
...
case "otc"
	sql = sql & "(OPERACIONES.ID_PROVINCIA <> 2) and (OPERACIONES.ID_PROVINCIA <> 3) and (Paises.id_region IN (1, 2, 3, 5)) )"	
case "otX"
	sql = sql & "(OPERACIONES.ID_PROVINCIA <> 2) and (OPERACIONES.ID_PROVINCIA <> 3) and (Paises.id_region<>1 and Paises.id_region<>2 and Paises.id_region<>3 and Paises.id_region<>5) "	
end select
...
sql = sql & "Paises ON OPERACIONES.ID_PAIS = Paises.Id "
	...
	sql = sql & "WHERE (web_es <>0) and (LOCALIDADES.id_provincia <> 0) and "
	sql = sql & "(OPERACIONES_DETALLE.seccion_operacion = 1) AND "
	...
	do until rsMadBcn.eof 
		if rsMadBcn("id_provincia")=2 then
			...
			if rsMadBcn("id_provincia")=50000 then
				ver_madbcn = "Otras Ciudades Internacional"
				madbcn = "otX"
			end if
...
<% function CalculaMEuros(p_op, p_sec, p_year, p_zona)
	...
	sql = sql & "(FECHA_OPERACION BETWEEN CONVERT(DATETIME, '" & p_year & "-01-01 00:00:00', 102) AND CONVERT(DATETIME, '" & p_year & "-12-31 00:00:00', 102)) AND "
	...
	case "otc"
		sql = sql & "( ID_PROVINCIA<> 3 and ID_PROVINCIA<> 2  ) "
	end select
...
<% sub CalculaEurosZ 
	...
	case "bcn"
		sql = sql & "(OPERACIONES.ID_PROVINCIA = 3) "
	case "otc"
		sql = sql & "( OPERACIONES.ID_PROVINCIA<> 3 and OPERACIONES.ID_PROVINCIA<> 2  ) "
	end select
