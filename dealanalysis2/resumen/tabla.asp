...
if request.Cookies("dev")<>"" then r_year = 2026
...
&nbsp; 
	...
	<select name="resumen_y" onChange="$('#frm_resumen').submit();" style="padding-left:5px; border:0px; font-family:'ruda',sans-serif; font-size:13px; font-weight:bold;background-color: transparent;"">
		<% for yy=2026 to 1996 step -1 %>
		<option <% if yy=r_year then %>selected<% end if %>><%= yy %></option>
		<% next %>
	</select>
	<select name="resumen_m" onChange="$('#frm_resumen').submit();" style="padding-left:5px; border:0px; font-family:'ruda',sans-serif; font-size:13px; font-weight:bold;background-color: transparent;"">
		<% dim mm 
...
$(document).ready( function() {
	$("#frm_resumen").submit(function(){
		$.ajax({
			type: "POST",
			url: "/dealanalysis2/resumen/datos.asp",
			data: $(this).serialize(),
			beforeSend: function(){
