...
r_month = request.Form("resumen_m")
r_month2=r_month+1
if r_month <10 then r_month="0"&r_month end if
if r_month2 <10 then r_month2="0"&r_month2 end if
...
sql = sql & "(OPERACIONES.FECHA_PUBLICACION BETWEEN CONVERT(DATETIME, '2026-"& r_month &"-01 00:00:00', 102) AND CONVERT(DATETIME, '2024-"& r_month2 &"-01 00:00:00', 102)) AND "
...
<% do until rs.EOF 
	...
	<% if request.Cookies("licencia")="" then %>
        <div class="fila_resum">
            <div class="resum_seccion"><a href="('<%= lcase(rs("seccion")) %>', '<%= r_year %>')" onclick="resumenSeccion('<%= lcase(rs("seccion")) %>', '<%= r_year %>',  '<%= r_month %>'); return false;"><%= lcase(rs("seccion")) %></a></div>
...
' WHEN LOCALIDADES.id_provincia = 2 THEN 2 ELSE CASE WHEN LOCALIDADES.id_provincia = 3 THEN 3 ELSE 10000 END END "
	...
	<% else %>
            <div class="resum_seccion madbcn" style="padding-left:5px;">&gt; <a href="('<%= p_sec %>', '<%= madbcn %>', '<%= r_year %>')" onclick="resumenMadBcn('<%= p_sec %>', '<%= madbcn %>', '<%= r_year %>', '<%= r_month %>'); return false;"><%= ver_madbcn %></a></div>
            <div class="resum_ops madbcn"><%= rsMadBcn("ops") %></div>
            <% if r_op="venta" then %><div class="resum_ops madbcn"><%= CalculaMEuros(r_op, p_sec, r_year, madbcn) %></div><% end if %>
            <div class="resum_superf madbcn"><%= formatnumber(rsMadBcn("sup"), 0) %></div>
	<% end if %>
...
<% function CalculaMEuros(p_op, p_sec, p_year, p_zona)
	...
	sql = sql & "(FECHA_PUBLICACION BETWEEN CONVERT(DATETIME, '2026-"& r_month &"-01 00:00:00', 102) AND CONVERT(DATETIME, '2024-"& r_month2 & "-01 00:00:00', 102)) AND "
...
function resumenSeccion (seccion, yy, mm1) {
	resetform();
	...
	$("#FechaI").val("01/"+ mm1 + "/" + yy);
	$("#FechaF").val("01/"+ (mm2) + "/" + yy);
	console.log($("#FechaF").val());
...
function resumenMadBcn(secc, madbcn, yy, mm1) {
	//$("#setcoords_zoom").val("");
	//$("#setcoords_lat").val("");
	//$("#setcoords_lng").val("");
	mm2=mm1+1;
...
<script>if (xOrigen==1) {resumenMadBcn('oficinas', 'mad', <%=r_year %>, <%=r_month %>);}</script>
