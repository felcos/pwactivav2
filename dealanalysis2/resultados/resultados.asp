...
<!--#include virtual="/dealanalysis2/inc/sql.asp" -->
...
if intermediario="%" and vendedor="%" and comprador="%" then 
	...
	sql = "SELECT *, seccion AS APARTADO FROM Operaciones_Agentes_FC WHERE (" & sqlW & ") "	

	maps_sql = "SELECT ID, ID_TIPO_OPERACION, METROS_CUADRADOS, SuperficieSR, SuperficieBR, PRECIO_EUR, ID_TIPO_PRECIO, TIPOPRECIO, "
	maps_sql = maps_sql & "inmueble, lat, lng, "
	maps_sql = maps_sql & "TIPODIRECCION, nombre_calle_pre, NOMBRE_CALLE, NUMERO_CALLE, NOMBRE_ZONA "
	maps_sql = maps_sql & "FROM Operaciones_Agentes_FC WHERE (" & sqlW & ") "
...
<form name="frm_opts" id="frm_opts" method="post" action="/dealanalysis2/resultados/resultados.asp" target="_blank" autocomplete="off">
	<% for each elto in request.form 
...
<div class="PwTabs">
    <ul class="nav nav-tabs lineNavs" style="" id="">
        ...
        <% if 1=1 then %><li <% if settab="map" then %>class="active"<% end if %>><a href="#map" data-id="map" data-toggle="tab" aria-expanded="true">Mapa</a></li><% end if %>
        ...
        <% if 1=1 then %><li <% if settab="distribucion" then %>class="active"<% end if %>><a href="#distribucion" data-id="distribucion" data-toggle="tab">Distribución</a></li><% end if %>
    </ul>
	...
	<div class="tab-content">
    	<div class="tab-pane <% if settab="list" then %>active<% end if %>" id="list" >
            ...
            <form name="frm_titulos" id="frm_titulos" method="post" action="/articulos/">
                ...
                <!--#include virtual="/dealanalysis2/resultados/inc_titulos.asp" -->
        ...
        <div class="tab-pane <% if settab="agencias" then %>active<% end if %>" id="agencias"><!--#include virtual="/dealanalysis2/resultados/inc_agencias.asp" --></div><!-- / Agencias -->
		
		<% if 1=1 then %>
        <div class="tab-pane <% if settab="distribucion" then %>active<% end if %>" id="distribucion"><!--#include virtual="/dealanalysis2/resultados/inc_distribucion.asp" --></div>
        <% end if %><!-- / Distribución -->
...
<% if swCargarMapa then %>
	...
	$.ajax({
		url: "/dealanalysis2/resultados/q_agencias.asp",
		data: "sqlw=<%= sqlW %>",
		async:false,
	...
	function contenidoInfoBox( operacion ) {
		...
		<% end if %>
		//console.log(operacion["PRECIO_EUR"], operacion["TIPOPRECIO"], operacion["METROS_CUADRADOS"], " > ", importe.toLocaleString("es", { maximumFractionDigits: 2}), moneda)
