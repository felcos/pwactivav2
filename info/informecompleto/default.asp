...
<script>
  function pruebaDivAPdf() {
      var pdf = new jsPDF('p', 'mm', [430, 584]);
      source = $('#informe_pw_1')[0];
...
pdf.setProperties({
	title: 'INFORME',
	subject: 'PropertyWeb.eu',
	author: 'A.G.',
	keywords: 'PropertyWeb.eu',
	creator: 'A.G.'
      ...
      pdf.addHTML(
          source,
          margins.left, 
          margins.top, {  margin: {top: 10, right: 10, bottom: 10, left: 10}, 'pagesplit':true},
          function (dispose) { 
...
<title>PropertyWeb - InfoEdificio</title>
...
<button onclick="javascript:pruebaDivAPdf()" class=" btn-flotante button"><span>Descargar PDF </span></button>
...
<div id="informe_pw_1" style="background-color:white; width: 1100px; max-width: 1100px;"class="container">
     
      <h1  class="heading" style="text-align:center ;">Informe - PropertyWeb.eu</h1>
      <h1  class="heading" style="text-align:center ;">Madrid, <%=FormatDateTime(Date, 1)%></h1>
      <hr>

      <% select case seltipo
      case "edif" 
        %><!--#include virtual="/info/inc/edificio_informe.asp" --><%
      case "dir"
    ...
    <section id="s_inmueble" class="caja info">
      ...
      <div class="row detalles">
          <div class="historico clearfix" id="historico_articulos">
              <%
              frm_url = "/info/data/titulos_informe.asp"
      ...
      if acceso_seccion and instr(request.Form, "historico_")=0 then checkAll=true
          ...
          <input type="hidden" name="origen" value="edif:<%= rsInmueble("id") %>">
              ...
              <div class="col-sm-4 " id="bypassme">
                <ul class="">
              ...
              <% if acceso_seccion then %>
                      <div class="col-sm-4 bloqBotontes">
                          <button style="display:none;" id="ver_articulos" type="submit" class="btn btnAzul" ><span class="icon icon-arrow-right2"></span> Mostrar Articulos</button>
