...
<div class="inm_tbl cabecera">
    <div class="inm_row">
        ...
        <div class="inm_nombre"><span >Industrial/Logistica</span></div>
...
<div class="inm_row <% if var_tipo_busq="edif" then
	...
	<div class="inm_check">
		<%if rtipo="ni" then %>
<form class="pagsum_detalle" id="frm<%= counter %>" method="post" action="/info/edificiod/" data-id="<%= data_id %>">
	<% else %>
    ...
    case "edif" 
		...
		' de momento no quiero el nombre de la planta y ocupante porque el listado es solo el edificio'
''			if not(isnull(rsBusq("planta"))) and rsBusq("planta")<>"" then
''				if not(isnull(rsBusq("logo"))) and rsBusq("logo")<>"" then
''					c_nombre = rsBusq("nombre")+ ": "+rsBusq("planta")+" "+ rsBusq("ocupante")
''					c_nombre = rsBusq("nombre")+ ": "+ rsBusq("ocupante")
''				else
''					c_nombre = rsBusq("nombre")+ ": "+rsBusq("planta")
''				end if
''			else
...
''			end if
		...
		if len(rsBusq("NombreInquilino"))>1 then
		'	c_nombre=c_nombre & "<br> Inquilino:" & rsBusq("NombreInquilino")
		end if


		if len(rsBusq("Nombre_Empresa1"))>1 then

			if rsBusq("TipoOP")="P" then
		'		c_nombre=c_nombre & "<br>&nbsp;&nbsp;Vendedor/Arrendador: " & rsBusq("Nombre_Empresa1")
			end if
			if rsBusq("TipoOP")="PI" then
		'		c_nombre=c_nombre & "<br>&nbsp;&nbsp;Intermediario V/A: " & rsBusq("Nombre_Empresa1")
			end if
			if rsBusq("TipoOP")="C" then
		'	c_nombre=c_nombre & "<br>&nbsp;&nbsp;Comprador/Inquilino: " & rsBusq("Nombre_Empresa1")
			end if
			if rsBusq("TipoOP")="CI" then
		'		c_nombre=c_nombre & "<br>&nbsp;&nbsp;Intermediario C/A: " & rsBusq("Nombre_Empresa1")
			end if
		end if
...
<a href="/info/edificio/" onclick="$('#frm<%= counter %>').submit();return false;" <% if request.Cookies("dev")<>"" then %>data-toggle="popover" data-trigger="hover" title="<%= pop_title %>" data-content="<%= pop_content %>" data-placement="bottom" data-html="true"<% end if %>>
    ...
    <div class="inm_disp_total">
		<% 
		if xEnConstruccion then  %> &nbsp;<img src="/img/underc.png" width="25"/> <% end if 
	...
	'response.write("prop-else")
		...
		'response.write("case 2 ")
			...
			'end if
				...
				sqlw = sqlw & " OR ("
				sqlw = sqlw & "Nombre_Empresa1 COLLATE Latin1_General_CI_AI LIKE '%" & busqueda & "%' COLLATE Latin1_General_CI_AI OR "
				sqlw = sqlw & "Nombre_Empresa2 COLLATE Latin1_General_CI_AI LIKE '%" & busqueda & "%' COLLATE Latin1_General_CI_AI OR "
				sqlw = sqlw & "Nombre_Empresa3 COLLATE Latin1_General_CI_AI LIKE '%" & busqueda & "%' COLLATE Latin1_General_CI_AI"
				sqlw = sqlw & ")"

				sqlw = sqlw & " OR ("
				sqlw = sqlw & "NombreInquilino COLLATE Latin1_General_CI_AI LIKE '%" & busqueda & "%' COLLATE Latin1_General_CI_AI "
				sqlw = sqlw & ")"
			...
			sqlw = sqlw & " AND (id_tipo_edificio is not null "
			'sqlw = sqlw & " or id_tipo_edificio=2 "
			'sqlw = sqlw & " or id_tipo_edificio=3 "
			'sqlw = sqlw & " or id_tipo_edificio=4 "
			sqlw = sqlw & " or id_tipo_edificio=5 "
			sqlw = sqlw & " or id_tipo_edificio=6 "
			'sqlw = sqlw & " or id_tipo_edificio=7 "
			'sqlw = sqlw & " or id_tipo_edificio=10 "
			'sqlw = sqlw & " or id_tipo_edificio=11 "
			sqlw = sqlw & " or id_tipo_edificio=12 "
			sqlw = sqlw & " or id_tipo_edificio=14 "
			'sqlw = sqlw & " or id_tipo_edificio=15 "
			sqlw = sqlw & " or id_tipo_edificio=17 "
			'sqlw = sqlw & " or id_tipo_edificio=18 "
			'sqlw = sqlw & " or id_tipo_edificio=19 "
			sqlw = sqlw & "  )"
...
'23	LOCAL
			...
			'if acceso_seccion then 
				...
				'sql = "SELECT * FROM View_Inmuebles_Emp_Plantas_FC WHERE (" & sqlw & ")" 
				sql = "SELECT * FROM View_Inmuebles_Empresas_FC WHERE (" & sqlw & ")"  				

				sql = sql & " ORDER BY id_pais, id_provincia, id_localidad,  nombre_calc "

				'sql = sql & " ORDER BY id_pais, ISNULL(id_localidad,9999), id_provincia, id_localidad,  complejo_orden, es_complejo DESC, nombre_calc"
			...
			'sql_datos = "SELECT id, nombre_calc, lat,lng FROM dirs_w_inmuebles WHERE " & sqlw & " AND lat IS NOT NULL"
			sql_datos = "SELECT id, nombre_calc, lat,lng, en_proyecto, planta, subtotal, logo, es_complejo, complejo,nombre, complejo_orden, id_edificio  FROM View_Inmuebles_Emp_Plantas_FC WHERE " & sqlw & " AND lat IS NOT NULL "
			'sql_datos = "SELECT id, nombre_calc, lat,lng FROM View_Inmuebles_Empresas_FC WHERE " & sqlw & " AND lat IS NOT NULL "
...
'response.write(sql_datos)
		...
		'response.write("case 3")
			'if acceso_seccion then
				sql = "SELECT dir1, dir2, dir3, dir5, id_pais, pais, id_localidad, localidad, tipo_direccion, nombre_calle, numero_calle "
			'else
