...
<% function calcular_sql()	
	...
	case else			'rtipo	
		...
		case 2	'inmuebles, semejante ó Dir. exacta	
			...
			sql_datos = "SELECT id, nombre_calc, lat,lng, en_proyecto FROM dirs_w_inmuebles WHERE " & sqlw & " AND lat IS NOT NULL"
		...
		case 3	'direcciones semejante	
			'if acceso_seccion then
				sql = "SELECT dir1, dir2, dir3, dir5, id_pais, pais, id_localidad, localidad, tipo_direccion, nombre_calle, numero_calle, fecha_renov, fecha_edif "
			'else
		...
		case 4	'zonas semejantes	
			'if session("pw_ws").accesoInfoEdificio and ini=0 then
				sql = "SELECT dir4, localidad, id_tipo_zona, tipo_zona, nombre_zona, id_pais, pais, fecha_renov, fecha_edif "
			'else
			'	sql = "SELECT COUNT(*) AS nn "
			'end if
