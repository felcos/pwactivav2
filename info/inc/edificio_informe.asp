...
if not isnull(rsInmueble("fecha_edifM")) then
    c_fecha_edif=(rsInmueble("fecha_edif")*100)+rsInmueble("fecha_edifM")
end if


if not isnull(rsInmueble("fecha_renov")) then
    c_fecha_renov=(rsInmueble("fecha_renov"))
else
    c_fecha_renov=0
end if
if not isnull(rsInmueble("fecha_renovM")) then
    c_fecha_renov=(rsInmueble("fecha_renov")*100)+rsInmueble("fecha_renovM")
end if
...
<div class="row" >
    ...
    <div class="col-sm-5">
        <!--#include virtual="/info/inc/shared/agentes.asp" -->
        		...
        		<!--#include virtual="/info/inc/shared/complejo2.asp" -->
    			</div></div>
                <% end if %>
...
<div class="detalles clearfix" >
      ...
      <!--#include virtual="/info/inc/shared/superficies_informe.asp" -->
            ...
            <% if acceso_seccion then
                ...
                <div class="descripcion">
                    ...
                    <% if not isnull(rsInmueble("fecha_edif")) then %>
                        <li                     
                            ...
                            ><span>A&ntilde;o Construcci&oacute;n :</span><span>
                                    ...
                                    if not isnull(rsInmueble("fecha_edifM")) then response.write("/"& rsInmueble("fecha_edifM"))  end if 
									end if %>
                    ...
                    <% if not isnull(rsInmueble("fecha_renov")) then %>
                        ...
                        if c_fecha_renov>ActualYYYY2 then  %> style="background-color:#CCCCCC;" <% end if 
                        else 
                        if c_fecha_renov>ActualYYYY then  %> style="background-color:#CCCCCC;" <% end if 
                        end if %>
                        ><span>A&ntilde;o Renovaci&oacute;n :</span><span>
                                <% if c_fecha_renov>10000 then
                                response.write mid(c_fecha_renov,5,2) + "/" + mid(c_fecha_renov,1,4)
                                else
                                response.write rsInmueble("fecha_renov")
                                if not isnull(rsInmueble("fecha_edifM")) then response.write("/"& rsInmueble("fecha_edifM"))  end if 
                                end if %>
                            
                            <% if c_fecha_renov>10000 then 
                            if c_fecha_renov>ActualYYYY2 then  %> &nbsp;<img src="/img/underc.png" width="25"/> <% end if 
                            else 
                            if c_fecha_renov>ActualYYYY then  %> &nbsp;<img src="/img/underc.png" width="25"/> <% end if 
                            end if %>
                        </span></li>
                        <% end if %>
                    <li><span class="separador02"></span><span  class="separador02"></span></li>
...
insert_reg_articulo "edif", "edif", rsInmueble("id")
